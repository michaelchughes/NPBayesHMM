function [] = plotStats( jobID, taskID )
...
if isfield( S(1), 'FRJ' )
    ...
    for iterID = 1:length( S );
        RJ = S(iterID).FRJ;
        nTryBirth(iterID) = RJ.ADD.nTotal;
        nAccBirth(iterID) = RJ.ADD.nAccept;
        
        nTryDeath(iterID) = RJ.DEL.nTotal;
        nAccDeath(iterID) = RJ.DEL.nAccept;
    end
...
if isfield( S(1), 'SM' )
    ...
    for iterID = 1:length( S );
        SM = S(iterID).SM;
        nTryM(iterID) = SM.DEL.nTotal;
        nAccM(iterID) = SM.DEL.nAccept;
        
        nTryS(iterID) = SM.ADD.nTotal;
        nAccS(iterID) = SM.ADD.nAccept;
    end
